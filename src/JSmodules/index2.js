...
//export and call the exported functions inside main()
//TODO: move to display estimates file
...
function covid19Estimator(){
    let  reportedCases, totalHospitalBeds, avgDailyIncomeUSD

    const impact = {
    currentlyInfected: reportedCases * 10,
    infectionsByRequestedTime: (requestedTimeInDays) => {
        return impact.currentlyInfected * 2 ** Math.floor(requestedTimeInDays / 3);
    },
    ...
    infectionsByRequestedTime: (requestedTimeInDays) => {
        return (
        severeImpact.currentlyInfected * 2 ** Math.floor(requestedTimeInDays / 3)
        );
    },
    severeCasesByRequestedTime: (requestedTimeInDays) => {
